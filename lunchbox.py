# -*- coding: utf-8 -*-
"""
Created on Tue Jun  4 19:12:06 2019
...
# SIGNATE お弁当の需要予測
import os
os.chdir("C:\\Users\\Eri\\Documents\\SIGNATE\\BENTO")
...
menu[menu['name'].str.contains(dic.iloc[0,1])]
meat = ['鶏','豚','牛','とり','トリ']
fish = ['サーモン',0,0,0,0]
np.vstack(meat,fish)
pd.DataFrame((meat,fish))
food = meat.extend(fish)
np.array(meat,fish)
pd.concat(meat,fish)
dict(zip(meat,fish))

# まずはお肉！
tori = menu[menu['name'].str.contains(dic.loc[0])]
buta = menu_best[menu_best['name'].str.contains('豚')]
beef = menu_best[menu_best['name'].str.contains('牛')]


# カレー。カレーは末尾に「カレー」がつくものとする。
curry = menu_best[menu_best['name'].str.endswith('カレー')]

# うどん・ご飯
udon_rice = menu_best[menu_best['name'].str.contains('うどん') |
        menu_best['name'].str.contains('ご飯')]

# 魚
fish = menu_best[menu_best['name'].str.contains('サーモン')|
        menu_best['name'].str.contains('魚')]
fish_recipe = ['サーモン','魚']
fish = menu
